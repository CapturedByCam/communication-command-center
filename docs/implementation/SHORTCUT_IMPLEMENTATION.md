...
Consult [V1 execution](V1_EXECUTION.md) and [PMC Current State](../../PMC/Current%20State.md) for the current private runtime and feature posture. The private macOS Shortcut and authenticated write-only endpoint passed synthetic device acceptance on September 23, 2026; intake was returned to false. The installed shell allocates one UUID before its two-attempt request loop and reuses the same payload for its single automatic transport retry. The setup-blocked export remains only a token-free recovery artifact, and the working token-bearing Shortcut must not be exported. iPhone/iPad installation remains unaccepted. Keep no-send, no-raw-content, and review-only normalization behavior unchanged.
