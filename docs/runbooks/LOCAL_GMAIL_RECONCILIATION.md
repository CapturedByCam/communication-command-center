...
The initial live pilot window is seven days. `CCC_GMAIL_LOOKBACK_DAYS` accepts only `7` or `30` and defaults to `7`; a separate 30-day backfill is deferred until the pilot is accepted. Each scan pins its window. A changed setting does not replace an incomplete checkpoint; the owner-run window-start operation requires Gmail intake to be off and preserves Queue and Audit rows. After completion, a new window overlaps the prior completion by one day, capped at the configured lookback. One invocation fetches at most one page and processes at most `batchSize` references (default 20, maximum 100). Each full-thread fixture snapshot is capped at 1,000 messages. Exceeding a bound is an explicit failure; snapshots must never be silently truncated by a future reader.

### Native Apps Script metadata pilot caveat

Advanced Gmail `Threads.get` can return older thread messages outside the requested search window. The Apps Script runtime never calls that endpoint. It constructs a one-message snapshot from the requested `Messages.get` metadata record only, inside the pinned seven-day pilot or an accepted 30-day backfill. That partial snapshot is explicitly generic (`ambiguous` / `other` / `review_only`, confidence `0`) and must not be treated as complete thread chronology, interpreted content, or a basis for drafting. Full-thread chronology and model interpretation remain blocked until a provider-supported bounded thread-read design is approved.
...
An overlap beyond one day is not a guarantee against arbitrarily late source indexing. Recovery is limited by the configured window (seven days during the pilot, at most 30 days after acceptance); longer gaps require a separately approved scope change.
...
The 2026-09-22 standing authorization covers minimum in-scope Google permissions, private deployment and bounded pilot tests. Record exact scopes, approved account, seven-day pilot query bounds, redaction and rollback before exercising each operation; the separate 30-day backfill is deferred until the pilot is accepted. Do not repeat approval for the same authorized scope. Workspace Studio, Shortcut installation and live feature acceptance remain open as detailed in the V1 execution record. No Google email or Chat message may be sent.
